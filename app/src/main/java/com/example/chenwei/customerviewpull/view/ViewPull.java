package com.example.chenwei.customerviewpull.view;
...
import com.example.chenwei.customerviewpull.R;

public class ViewPull extends View {
    //圆的画笔
    ...
    public ViewPull(Context context) {
        super(context);
        init(null);

    }

    public ViewPull(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init(attrs);
    }

    public ViewPull(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(attrs);
    }

    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    public ViewPull(Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
...
//        super.onMeasure(widthMeasureSpec, heightMeasureSpec);
        ...
        int iHeight=(int)((mDragHeight*mProgress+0.5)+ 2*mCircleRadius+getPaddingTop()+getPaddingBottom());
        ...
        if(heightMode==MeasureSpec.EXACTLY){
            ...
            measureHeigh=Math.min(iHeight,height);
        }else{
            measureHeigh=iHeight;
        }
